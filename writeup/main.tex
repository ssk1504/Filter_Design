...
We are supposed to design the equivalent FIR and IIR filter realizations for filter number 10.  
...
to \{4 + 0.6(j+2)\}kHz.  Since our filter number is 10, substituting $j = 10$ gives the passband
range for our bandpass filter as $10$ kHz - $11.2$ kHz.  Hence, the un-normalized discrete time filter
passband frequencies are $F_{p1} = 11.2$ kHz
and $F_{p2} = 10.0$ kHz.  The corresponding normalized digital filter passband frequencies are
$\omega_{p1} = 2\pi\frac{F_{p1}}{F_s}  = 0.47\pi$  and $\omega_{p2} = 2\pi\frac{F_{p2}}{F_s}  = 0.42 \pi$ kHz.  The centre frequency is then given by  $\omega_c = \frac{\omega_{p1} + \omega_{p2}}{2} = 0.445\pi$.  

\item {\em Stopband:}  The {\em transition band} for bandpass filters is $\Delta F = 0.3$ kHz on either side of the passband.
Hence, the un-normalized {\em stopband} frequencies are $F_{s1} = 11.2 + 0.3 = 11.5$ kHz and $F	_{s2} = 10.0 - 0.3 = 9.7$ kHz.  The corresponding normalized frequencies are $\omega_{s1} = 0.4792 \pi$  and $\omega_{s2} =  0.4042 \pi$.
\end{enumerate}

\subsection{The Analog filter}
In the bilinear transform, the analog filter frequency ($\Omega$) is related to the corresponding digital filter frequency ($\omega$) as $\Omega = \tan \frac{\omega}{2}$.  Using this relation, we obtain the analog passband and stopband frequencies as $\Omega_{p1} = 0.9004$, $\Omega_{p2} = 0.7673$ and $\Omega_{s1} = 0.9366$, $\Omega_{s2} = 0.7366$
respectively.
...
%\end{equation}
where $\Omega_0 = \sqrt{\Omega_{p1}\Omega_{p2}} = 0.8312$ and $B = \Omega_{p1} - \Omega_{p2} = 0.1331$.  The low pass filter has
the passband edge at $\Omega_{Lp} = 1$ and stopband edges at $\Omega_{Ls_1} = 1.4949$ and $\Omega_{Ls_2} = -1.5133$.  We choose the stopband edge of the analog low pass filter as $\Omega_{Ls} = \mbox{min}(\vert \Omega_{Ls_1}\vert,\vert \Omega_{Ls_2}\vert) = 1.4949$.
...
we obtain $N \geq 4$ and $0.2857 \leq \epsilon \leq 0.6197$.  In Figure 1, we plot $\vert H(j\Omega)\vert$ for a range of values of $\epsilon$, for $N = 4$.  We find that for larger values of $\epsilon$, $|H(j\Omega)|$ decreases in the transition band.  We choose $\epsilon = 0.4$  for our IIR filter design.  
...
\begin{equation}
\label{bpfinal}
H_{a,BP}(s) = \frac{8.7661\times 10^{-5}s^4}{s^8 + 0.1473s^7 + 2.7921s^6 + 0.3075s^5 + 2.9035s^4 + 0.2124s^3 + 1.3327s^2 + 0.0485s + 0.2278}
\end{equation}
}
...
\end{enumerate}
\begin{figure}
\includegraphics[width=1.3\linewidth]{figs/fig1.eps}
...
\begin{figure}
    \centering
    \includegraphics[width=1.5\linewidth]{figs/fig2.eps}
    \caption{The magnitude response plots from the specifications in Equation 5 and the design in Equation 9}
    \label{fig:fig2}
\end{figure}
...
\begin{figure}
\label{fig3}
\includegraphics[width = 15cm]{figs/fig3.eps}
...
and
\begin{eqnarray}
D(z) = 8.8720  -12.6900z^{-1} + 40.7472z^{-2}  -38.1149z^{-3}+  62.1390z^{-4}\nonumber \\
  -36.7008z^{-5}+   37.7819z^{-6}  -11.3278z^{-7} +   7.6259z^{-8}
\end{eqnarray}
...
\begin{figure}
\label{fig4}
\includegraphics[width = 15cm]{figs/fig4.eps}
...
\begin{figure}
\label{fig5}
\includegraphics[width = 15cm]{figs/fig5.eps}
...
\begin{figure}
\label{fig6}
\includegraphics[width = 15cm]{figs/fig6.eps}
